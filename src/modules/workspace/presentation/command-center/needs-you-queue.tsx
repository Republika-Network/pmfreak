...
export function NeedsYouQueue({
  ...
  // A host may mount this component more than once across responsive surfaces, so a document-global id would appear twice and `aria-labelledby` would
    ...
    <section aria-labelledby={headingId} data-testid="cc-section-needs-you">
      <div className="flex items-center justify-between gap-2 px-1">
        ...
        {!loading && !errorMessage && !incomplete && items.length > 0 && (
          <span className="shrink-0 text-[11px] text-slate-500">{items.length}</span>
        )}
      </div>
      ...
      <div role="status" aria-live="polite">
        ...
        {(loading || incomplete) && !errorMessage && incompleteNote && (
          <p className="mt-2 px-1 text-[11px] text-slate-500" data-testid="cc-attention-incomplete">
            {incompleteNote}
          </p>
        )}
        {errorMessage && (
      ...
      {groups.map((group) => (
        <section key={group.job} aria-labelledby={`${headingId}-${group.job}`} data-testid={`cc-attention-group-${group.job}`} className="mt-3">
          ...
          <h3
            ...
            className="px-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500"
          >
            {group.label}
          </h3>
