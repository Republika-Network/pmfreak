...
export function MonitoringPanel({
  ...
  return (
    <section aria-labelledby={headingId} data-testid="cc-section-monitoring">
          ...
          <div className="mt-2 rounded-xl border border-rose-500/25 bg-rose-500/[0.08] px-3 py-3">
            ...
            {onRetry && (
              <button
                type="button"
                onClick={onRetry}
                className="mt-2 rounded-lg border border-rose-500/30 bg-rose-500/10 px-2.5 py-1.5 text-xs font-medium text-rose-900 transition hover:bg-rose-500/20"
              >
                Try again
              </button>
            )}
      ...
      {showCoverage && (
        <ul className="mt-2 space-y-0.5 rounded-xl border border-slate-200 bg-white px-3 py-2.5">
          {summary.areas.map((area) => (
            <li key={area.id} data-testid="cc-monitoring-area" className="flex items-center justify-between gap-2 py-0.5">
              <span className="flex min-w-0 items-center gap-2">
                ...
                <span className="truncate text-sm text-slate-700">{area.label}</span>
              </span>
              <span className={`shrink-0 text-[11px] ${area.recentSignalCount > 0 ? toneStyles(area.tone).text : "text-slate-500"}`}>
                {area.statusLabel}
              </span>
            </li>
          ))}
      ...
      {showCoverage && (
        <p className="mt-1.5 px-1 text-[11px] text-slate-500" data-testid="cc-monitoring-scope">
      ...
      {detail && !errorMessage && (
        <details className="mt-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2" data-testid="cc-monitoring-detail">
