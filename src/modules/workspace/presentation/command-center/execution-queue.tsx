  ...
  return (
    <section aria-labelledby={headingId} data-testid="cc-section-in-progress">
      <div className="flex items-center justify-between gap-2 px-1">
        ...
        {!loading && inProgress.length > 0 && <span className="shrink-0 text-[11px] text-zinc-500">{inProgress.length}</span>}
      </div>
      ...
      {inProgress.length > 0 && (
        <ul className="mt-2 space-y-1.5">
          {inProgress.map((chain) => (
            <ChainRow key={chain.id} chain={chain} onSelect={onSelect} testId="cc-in-progress-item" />
          ))}
        </ul>
      )}
        ...
        <div className="mt-3" data-testid="cc-not-progressing-group">
          ...
          <ul className="mt-2 space-y-1.5">
            {notProgressing.map((chain) => (
              <ChainRow key={chain.id} chain={chain} onSelect={onSelect} testId="cc-not-progressing-item" />
            ))}
          </ul>
        </div>
      )}
      ...
      {!loading && closed.length > 0 && (
        <details className="mt-3 rounded-xl border border-white/10 bg-white/[0.02] px-3 py-2" data-testid="cc-closed-chains">
          ...
          <ul className="mt-2 space-y-1.5">
            {closed.map((chain) => (
              <ChainRow key={chain.id} chain={chain} onSelect={onSelect} testId="cc-closed-chain-item" />
            ))}
          </ul>
        </details>
      )}
