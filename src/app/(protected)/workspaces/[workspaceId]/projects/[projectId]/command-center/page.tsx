...
export default async function ProjectCommandCenterPage({
  ...
  const { data: pmoRow } = project.pmo_id
    ? await supabase
        .from("pmos")
        .select("id, name")
    ...
    : { data: null };
  const pmo: ProjectBreadcrumbPmo = pmoRow ?? null;
              ...
              {recommendations.map((item) => (
                <li key={item.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  {item.confidence_score !== null ? (
                    ...
                    <p className="mt-1 text-[11px] text-zinc-500">Generated by PMFreak</p>
                  )}
                </li>
              ))}
            </ul>
        ...
        {governance === null ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              { label: "Awaiting a governance decision", value: governance.decisionRequiredCount, tone: "text-amber-800" },
