...
import { pmoReportsPath } from "@/lib/pmos/pmo-paths";
...
/**
 * Legacy PMO Reports entry point — a resolver, not a screen.
 *
 * PMO Reports moved to `/workspaces/[workspaceId]/pmos/[pmoId]/reports`. This path
 ...
 */
export default async function LegacyPmoReportsRedirectPage({ params }: Props) {
  const user = await requireAuthUser();
  const { pmoId } = await params;

  const access = await resolveLegacyPmoRoute(user.id, pmoId);
  if (access.access === "denied") {
    console.error(JSON.stringify({ event: "legacy_pmo_reports.pmo_not_accessible", userId: user.id, requestedPmoId: pmoId }));
    return <PmoNotAvailable />;
  }

  redirect(pmoReportsPath(access.workspaceId, access.pmoId));
}
