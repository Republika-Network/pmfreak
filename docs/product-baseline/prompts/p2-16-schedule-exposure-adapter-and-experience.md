...
- **Adapter:** a typed H7/H8 change (`dependency_change` or `milestone_date_change` — *historical: that milestone trigger was replaced by the honest `milestone_state_evaluation` in the final SHA; see the 2026-09-21 section* — resolved from persisted rows only) is evaluated by the UNCHANGED H9 engine (`validateGraph → forwardPass → backwardPass → computeFloat → computeCriticalPath → computeCriticalMilestones`) over a content-addressed snapshot (`schedule-snapshot:v1`, no clock), then recorded as Raw Input → `schedule_exposure.evaluated` v1 Normalized Event → `INFERENCE`/`RISK` Evidence → `schedule_risk` Finding → governed `proposed` Recommendation (`source_signal_id`). No Decision, Action, Task, Outcome or Observation is created.
...
## Review reopening and final verification — 2026-09-21

The 2026-09-20 record above stands as written: `b602363e` was genuinely verified for that candidate. Review then found defects in it, which reopened P2-16; this section records what happened next rather than rewriting the earlier result.

- **Chronology:** initial candidate `b602363ec5f3536a16fdfc1a6368ffd004b8f027` (verified) → docs `1e4c510e8fff6d8c5ca39154bf255aad62cbd53b` → CodeQL test-only fix `ba2ef081d17a773b038510629fd0c7cac02b7d00` (`js/double-escaping` in a test helper; alert #17 fixed) → Codex review of `ba2ef081` found five valid defects and P2-16 was reopened → all five repaired in `d001345dbf6a70e362e9d0abbc4c8d2625ef5246` → **final executable verification SHA `d001345dbf6a70e362e9d0abbc4c8d2625ef5246`**. `b602363e`, `1e4c510e` and `ba2ef081` are historical chronology.
- **#1 (P1) trusted engine boundary.** Any owner/admin/PM could call the capture RPC directly and record hand-written output attributed to the deterministic engine. All three adapter RPCs are now `service_role`-only (PUBLIC/anon/authenticated revoked) and refuse any other caller in-body; they are reached only from the authenticated route through a registered privileged transport (`src/lib/critical-path/schedule-exposure-trusted-writer.ts`) after the server has read the schedule through the user's RLS client and run H9 itself. Every call names the authenticated human; each function re-verifies that actor's current owner/admin/pm membership of the exact workspace + project, and attribution is that human, never the service role. Security-definer totals: authenticated 23, service_role 33.
- **#2 concurrency.** Materialisation takes a per-Evidence `pg_advisory_xact_lock` before its existence check.
- **#3 snapshot identity.** Task and milestone titles, which the payload renders, are bound into `schedule-snapshot:v1`; a rename is a new identity, never an idempotency conflict.
- **#4 honest milestone trigger.** `milestone_date_change` is replaced by `milestone_state_evaluation`: an explicit evaluation of the milestone's current state. `updated_at` is kept as `sourceUpdatedAt` provenance only and `occurredAt` is the evaluation, because H8 records no date-change history. `dependency_change` remains a true typed change backed by the H7 event.
- **#5 recovery.** The projection reports `materializationState` (`complete` only with Evidence + Finding + Recommendation); an incomplete chain renders as an alert, and a write-authorised, project-scoped server resume re-materialises that exact persisted Evidence.
- **Load-bearing proof (mutation-checked):** re-granting `authenticated` and removing the in-body guard fails the gate (a same-project PM's direct capture reached the function body); removing the advisory lock fails the concurrency proof 3/3 runs; removing titles from the snapshot fails both identity tests.
- **Environment:** authoritative migration verification on a fresh isolated Supabase stack (separate project id and ports) applying 167/167 migrations from zero; the P2-14 harness pins the canonical local ports, so its regression ran on the standard local stack after its three stale pre-review P2-16 overloads were dropped and the final migration applied (verified: only the final service_role-only signatures remained).
- **Full final-tree battery (before commit, identical tree):** P2-16 behavioural 34/34; P2-16 UI 12/12; critical-path minimum 159/159; `check:p2-16-db` PASS (200 assertions; 12 concurrent materialisations → created 1, duplicate 11; Finding/risk/governance/Recommendation/agent_run/agent_output 1 each); `check:operational-flow-db` ok; `check:fresh-db-migrations` PASS on a pristine stack (167/167, 433 tables, 33 SECURITY DEFINER — PUBLIC 0, anon 0, authenticated 23, service_role 33); `check:security-definer-hardening` PASS (33); `check:aoc-boundaries` and `check:no-local-auth-bypass` PASS; typecheck 0; lint 0 errors / 643 warnings (= baseline); build exit 0; `npm test` 14,492 pass / 0 fail / 23 skipped; Chromium P2-16 11/11 and P2-14 journey + session continuity 38/38.
- **Exact-head proof on `d001345d`** (clean Linux checkout, `git status` empty; committed migration re-applied to the fresh isolated stack): P2-16 behavioural 34/34; P2-16 UI 12/12; critical-path minimum 159/159; `check:p2-16-db` PASS (200 assertions); `check:security-definer-hardening` PASS (33); `check:operational-flow-db` ok; typecheck 0; `git diff --check` clean (tree and range); Chromium P2-16 11/11.
- **Known limitations carried forward:** the engine limitations and the residual P2-09 interaction recorded above are unchanged. The privileged transport emits a `privileged_client_used` security event per write request by design.
- **Merged:** PR #616 was rebase-merged, so the SHAs above were replayed onto `main` under new ids (final executable `d001345d` → `131f46b5dde2bbe9bc88a131bfe1796e598bdcc4`; final record `553ab12d` → `cc16d1f785cac7e573f0975e5fccdbe502180a0b`), each with an identical tree; the full map is in `PMFREAK_SEQUENTIAL_BUILD_PLAN_P2.md`. The fresh Codex review of the final head was not completed (usage limit); an alternative independent final-head review was accepted.
