...
PR #614 review found a second defect on the same Finding path this prompt repaired, and it is recorded here rather than folded into the evidence above.

- **Defect:** `operational_signals.confidence_score` is `numeric(5,2)` on a 0-100 scale (the detector writes 92; the risk derivation compares `>= 85`), while Evidence and Observation confidence are `numeric(5,4)` fractions and every consumer of `LineageStepNode.confidenceScore` multiplies by 100. The Finding projection treated the percentage as a fraction, so a live Signal of 92 rendered `9200.0%` in the step summary and again in the UI. It was unreachable until the `source_signal_id` repair made the Finding resolve, which is why this branch surfaced it.
- **Repair:** normalize at the single Signal → `LineageStepNode` boundary; the schema and the persisted 0-100 Signal contract are untouched, and no consumer was taught to infer a scale. Three drifted Signal fixtures (0.91/0.95) were corrected to production-shaped values; Evidence and Observation fixtures keep their 0-1 contract.
- **Proof:** live, on a browser-created chain — persisted `95.00` → projected `0.95` → rendered `95.0%`, with the lineage still `complete` and `gapCount: 0`. P2-10 51/51, P2-20 50/50; removing the normalization fails two P2-10 tests and the P2-20 export test.
- **Final exact executable verification SHA:** `75816028089a0296b1216372c3d9c45f90cb83bf` (C7). The evidence sections above were verified on `0fd86b561326c7895980fffb5c47c8aa6b8585c5` (C4) and remain accurate for that candidate. PR #614 was rebase-merged, so both ids were replayed onto `main` under new SHAs (C7 → `7cd64e9cded628b7cc3d70efd2d09941729e4ea6`, C4 → `97d7d59873fde2a67f215b4f49b0d1622831b91a`); the full map is in `PMFREAK_SEQUENTIAL_BUILD_PLAN_P2.md`.
