...
- **Release and governance gates, all PASS:** `check:governance` (full chain), `check:aoc-boundaries`, `check:no-local-auth-bypass`, `check:frontera-consumer`, `check:security-definer-hardening` (30 functions), `check:release-readiness`, `compliance:check`, and `check:fresh-db-migrations` — **166/166** migrations applied from empty on an independent disposable Postgres, 433 tables, 30 SECURITY DEFINER functions, 1 pre-existing table without RLS. The count moved 165 → 166 because C4 adds the terminal-revocation migration; the 2026-09-19 section above records the 165-migration run that preceded it, and is left as it was.
