...
- **Executable verification SHA:** `fc4b2166` (`fix(learning): evaluate candidates at database time over valid sources`), final. Earlier candidates were superseded by review: `a32335ff` (initial implementation; service-role direct-write gap) and `b93dd9fb` (grant fix; six further review findings, below). Every result in this section was re-run on `fc4b2166`.
...
- **Eligibility:** the RPC is the sole authority and derives every rule from canonical rows at the database clock, never looser than the P2-10 lineage projection. There is no client-side preflight over the project-wide projection, which the PostgREST row cap could truncate. The Observation (named, or else the Outcome's latest) must be the latest, with a qualifying result (achieved/partial/failed). No Observation in the Outcome's history may be disputed, inconclusive, incomplete (PARTIAL/UNKNOWN) or fixture. The Observation must be unexpired, and its Evidence and the Finding's Evidence must both still meet P2-09's canonical-live predicate at the clock. Task and execution must be completed; the governance evaluation the execution ran under must be authorized/not_required, with no denied/revoked evaluation; the Decision must be accepted/modified and not superseded; the Recommendation, Finding and canonical LIVE Finding Evidence must be present and not degraded. Every failure is a named reason, and nothing is written.
...
- **Dedupe / versioning:** a retry of an already-linked Observation is a `duplicate` (no write, no event), judged before current eligibility, so a retry after the Observation lapses never reports a committed write as failed. A newer Observation supersedes the Outcome's previous source (kept, with `superseded_by`). A new Outcome of the same pattern adds a source. Every material change increments `version` and recomputes the summary and `evidence_digest` over the sources **valid at the evaluation clock**; a lapsed source is kept but not counted. Uniqueness is enforced by constraints, including one current source per Outcome, plus a per-pattern advisory lock.
...
- **Evaluation clock:** the RPC takes no clock parameter and evaluates at the database's `now()`, so eligibility and the emitted event cannot be backdated.
- **Migration:** `20260914000000_p2_18_learning_candidate_lineage.sql`, additive and forward-only. The RPC is `propose_canonical_learning_candidate(uuid, uuid, uuid, uuid)`. Grant matrix: 34 SECURITY DEFINER functions, 24 authenticated.
...
- **Second review round (six findings on `b93dd9fb`, all fixed in `fc4b2166`):**
  - (P1) The snapshot was recomputed over non-superseded sources, including lapsed ones; now only sources valid at the clock are counted.
  - (P1) The Finding's Evidence was checked for staleness and degradation only; it now meets the full canonical-live predicate at the clock.
  - (P1) The caller could backdate `p_evaluated_at`; the clock is now the database's.
  - (P2) A retry after expiry returned `ineligible`; it now returns `duplicate`.
  - (P2) The propose preflight read the project-wide projection, which a row cap could truncate; it was removed.
  - (P2) A valid-JSON non-object body caused a 500; it is now a 400.
...
- **Results:**
  - P2-18 behavioural/contract 18/18. The four projection-preflight tests went with the preflight; their eligibility rules are proven against the database by `check:p2-18-db`.
  ...
  - `check:p2-18-db` PASS (200 assertions, twice on `fc4b2166`; a retry after an Observation lapses is a duplicate; the recomputed snapshot excludes a lapsed source; a lineage whose Finding Evidence lapsed is refused, identically on repeat; a caller-supplied clock is rejected; direct `service_role` candidate/source inserts and updates denied, with the aggregate, sources and events unchanged; LIVE lineages built through the product API; retries and 10 concurrent retries are duplicates; 8 concurrent first proposals give 1 link + 7 duplicates; supersession keeps history; tiers single → multiple_consistent → conflicting; events equal material changes; viewer/outsider/IDOR refused; the service role cannot delete or rewrite lineage).
  ...
  - `npm test` 14,544 pass / 25 skipped; its 2 failures are the WSL worktree line-ending pair, which pass 7/7 under native Windows Git. Module mocks 19/19.
...
- **Known limitations:**
  ...
  - A lineage whose Finding Evidence has passed its own `stale_at` can no longer be proposed. This is P2-09's canonical-live predicate, applied as for the Observation's Evidence; sources already linked stay as history.
