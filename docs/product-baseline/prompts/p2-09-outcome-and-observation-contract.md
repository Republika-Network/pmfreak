...
- `npx tsx --test tests/decision-outcome-engine.test.mjs tests/agent-controlled-execution-result-reconciliation-human-outcome-review.test.mjs` — 180/180 pass.
- `tests/p2-09-outcome-observation-contract.test.ts` — 7/7 pass.
- `npm run check:p2-09-db` — PASS, 74 assertions (baseline 74). A completed Task leaves the Outcome `expected`; only an explicit LIVE evidence-backed Observation moves it. The Observation emits `canonical_outcome_learning_candidate.v1`. Fixture Evidence is refused.
- `npm run check:operational-flow-db` — PASS. `npm run check:aoc-boundaries` — PASS. `npm run check:no-local-auth-bypass` — PASS.
- `npm run typecheck` — PASS. `npm run lint` — 0 errors. `npm run build` — PASS. `git diff --check` — clean.
- Browser: the P2-14 journey (STEP_14–STEP_16b, 38/38, run twice) shows Task completion ≠ Outcome achievement, an explicit expected Outcome, an evidence-backed Observation and idempotent retry.
- **Residuals:** none specific to P2-09.
