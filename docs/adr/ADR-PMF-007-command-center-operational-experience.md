...
- ~~Remove the stray "Operational Command Center" `<h1>`~~ **Implemented:** the heading now reads "Workspace Chat", the established label for that surface (`/chat` calls itself "the workspace-level conversational console"). It was renamed rather than deleted so the section keeps a heading; "Command Center" as a user-facing phrase is no longer applied to it. Note the location recorded in the evidence list above was wrong — the heading was in `workspace-conversation-shell.tsx`, not on `/projects`. It was deliberately NOT renamed to "Workspace Command Center": that name belongs to the screen at `/workspaces/[workspaceId]/command-center`, and two surfaces answering to it would be the entity confusion this ADR ruled against.
